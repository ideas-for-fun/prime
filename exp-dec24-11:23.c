	...
	for (i = 127 ; count > 1 ; i--)
	{
		if (c2terms_powers[i])
		{
			...
			for (k = median ; xcount ; k--) 
			{
				if (c1terms_powers[k])
				{
					printf("\n\n Creating the summation polynomial %d %d\n", k, i);
					printf("\n\n Creating the summation polynomial %d \n", c2terms_powers[i] + c1terms_powers[k]);
...
// TODO : LAST STRETCH : This LOOP HAS A PROBLEM..... FIGURE OUT THE LOGIC AND FIX IT !!!
	...
	for ( i = 127 , ii = 0, k = 127, kk = 0; i ; i--, ii++)
	{
				...
				if (1)
				{
					...
					if (i < median)
						c2compute += ((__uint128_t) pow((double) 2, (double) ii)) / 
								((__uint128_t) pow((double) 2, (double) terms_compdiv[i]));
					else
